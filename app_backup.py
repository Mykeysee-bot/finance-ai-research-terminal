...
with st.sidebar:
    ...
    loaded_count = sum(MODULES.get(name) is not None for name in MODULE_NAMES)
    st.progress(loaded_count / len(MODULE_NAMES))
    st.caption(f"{loaded_count} of {len(MODULE_NAMES)} modules loaded")

    with st.expander("Module diagnostics"):
        st.dataframe(module_status_table(), use_container_width=True, hide_index=True)

    st.divider()
    st.markdown("### Project Snapshot")
    st.write("**Platform:** Python + Streamlit")
    st.write("**Market Data:** yfinance")
    st.write("**Visualization:** Plotly")
    st.write("**Core Features:** Research, scoring, statements, estimates, and DCF")
    st.caption(
        "Built as a portfolio project for equity research, valuation, "
...
st.title("AI Financial Research Terminal")
st.caption(
...
with overview_tab:
    ...
    if company_data and clean_ticker(str(first_present(company_data, "ticker", default=""))) == main_ticker:
        ...
        with st.expander("Raw market-data output"):
...
with comparison_tab:
    ...
    if st.button("Compare Companies", type="primary", key="compare_companies_button"):
        if not first_ticker or not second_ticker:
            ...
            try:
                with st.spinner(f"Comparing {first_ticker} and {second_ticker}..."):
                    ...
                    st.session_state.comparison_result = run_comparison(
                        first_company,
                        second_company,
                    )
                    st.session_state.ai_comparison_result = run_ai_comparison(
                        first_company,
                        second_company,
                        first_scores,
                        second_scores,
                ...
                st.error(f"Could not compare the companies: {exc}")
    ...
    if comparison_companies:
        ...
        with ai_comparison_tab:
            if st.session_state.ai_comparison_result is not None:
...
"""
    ...
    with project_metric_one:
        st.metric("Integrated Modules", f"{loaded_count}/{len(MODULE_NAMES)}")
    ...
    st.code(
        "Built an AI-powered public-equity research platform using Python and "
        "Streamlit that integrates market data, financial statements, "
        "multi-year trend analysis, analyst estimates, news intelligence, "
        "peer comparisons, financial scoring, investment recommendations, "
        "and DCF valuation for publicly traded companies.",
        language=None,
    )
...
st.divider()
st.caption(
    "Finance AI Agent · Data may be delayed or incomplete. "
